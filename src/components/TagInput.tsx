...
export function TagInput({ value, onChange, allTags, placeholder = "Add tag…" }: TagInputProps) {
    ...
    <Popover.Root open={open && suggestions.length > 0} onOpenChange={setOpen}>
      ...
      <Popover.Portal>
        <Popover.Content
          ...
          align="start"
          sideOffset={4}
          className="z-50 w-[var(--radix-popover-trigger-width)] rounded-md border border-edge bg-surface shadow-lg"
        >
          {suggestions.map((tag) => (
            <button
              key={tag}
